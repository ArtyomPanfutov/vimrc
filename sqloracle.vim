...
" 17.01.2018: Add Diasoft's keywords
...
" Diasoft's keywords
syn keyword sqlKeyword  __BEGIN_PROCEDURE__ M_DELETE_PTABLE DROP_CREATE M_BUSINESSLOG_BEGIN 
syn keyword sqlKeyword  M_BUSINESSLOG_CHECKPOINT M_FORCEORDER M_LOG_TABLE M_KEEPPLAN M_ISOLAT
syn keyword sqlKeyword  M_BUSINESSLOG_PARAM __COMMIT_TRAN__ __END_PROCEDURE__ X_ANYMODE __ERR_TRAN__
syn keyword sqlKeyword  SET_IDENTITY_LOCK DEL_IDENTITY_LOCK GET_M_ID M_WITH_ROWLOCK M_P_WITH_ROWLOCK 
syn keyword sqlKeyword  M_NOLOCK_INDEX API_CREATE_PROC M_BUSINESSLOG_BLOCK_BEGIN M_BUSINESSLOG_BLOCK_END
syn keyword sqlKeyword  M_DELETE_PTABLE_SPID_INDEX M_ROWLOCK_INDEX __DECLARE_CURSOR__ __FETCH_STATUS__ __FETCH_NEXT__
syn keyword sqlKeyword  NULLDATE BEGIN_TRANSACTION_VAR COMMIT_TRANSACTION_VAR ROLLBACK_TRANSACTION_VAR BEGIN_TRANSACTION_ 
syn keyword sqlKeyword  SAVE_TRANSACTION_ COMMIT_TRANSACTION_ ROLLBACK_TRANSACTION_ SYB_STYLE
syn keyword sqlKeyword  __DEALLOCATE_CURSOR__ M_UPDLOCK_INDEX M_KEEPPLAN_FAST M_INDEX M_HOLDLOCK_INDEX M_INMEMTBL
syn keyword sqlKeyword  #define #include #ifndef #ifdef #endif #elseif API_SERVICE_EXISTS _IF _YES _NO _END API_EVENT_EXISTS
syn keyword sqlKeyword  isnull declare @@spid DCL_PROC_BEGIN @@rowcount @@trancount 
...
syn keyword sqlStatement rename revoke rollback savepoint set rowcount 
syn keyword sqlStatement truncate
" next ones are contained, so folding works.
syn keyword sqlStatement create update alter select insert contained (select 
syn keyword sqlStatement go goto exec  
...
syn keyword sqlType	smallint real timestamp urowid varchar varchar2 varray smalldatetime image
" Diasoft datatypes
syn keyword sqlType     DSIDENTIFIER DSOPERDAY DSINT_KEY DSSMALLINT DSBRIEFNAME 
syn keyword sqlType     DSFULLNAME DSACCNUMBER DSCOMMENT DSTINYINT DSDATETIME
syn keyword sqlType     DSFLOAT DSSPID DSLABEL DSMEMO DSVARFULLNAME DSUUID
syn keyword sqlType     DSUSERNAME DSMONEY DSBIGMONEY DSACC_SWIFT 
" Strings:
